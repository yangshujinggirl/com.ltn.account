              ...
              <div className={classStaus} key={index}>
                <div className="l-ac">
                  <div className="tick-head clear">
                    <span className="name">{el.activityType}</span>
                    {
                      status=='A'?
                      <span
                        className="send"
...
class RaiseTickitPage extends React.Component {
  constructor(props) {
    super(props);
    this.state ={
      dataList:[],
  ...
  getTickitsTab(type,current) {
    ...
    raiseTickData(trans[type],current-1)
    .then((data)=>{
      this.setState({
  ...
  paginationEvent(page, pageSize) {
    ...
    this.getTradeListTab(this.state.tabShow,page);
  }
  goExcharge() {
    location.hash=`/account/exchange?backUrl=${this.props.match.path}`;
  }
  render() {
    const { dataList,tabShow,current,totalCount }=this.state;
    return(
      <AccountWrapModule itemId='11' {...this.props}>
        <div className="main-pages-wrap return-tickit-page">
          <Head title="加息券"/>
          <div className="content-action">
            <div className="get-tickit-way">
              <p className="way-title">获取加息券方式：</p>
              <div className="list">
                ...
                <a href="/other/mypartner" className="item item-partner">
                  <div className="icon"></div>
            ...
            <div className="tips-content">
              <p className="item">
                1.加息券：奖励给投资者的一种加息券，投资时使用，标的到期利息按照加息后算，用户可继续投资或提现。
              </p>
              <p className="item">
                2.加息券使用：
...
export default RaiseTickitPage;
